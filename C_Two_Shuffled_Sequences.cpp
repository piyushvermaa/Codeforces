...
void solve(){
    int n; cin>>n;
    vi arrrr(n);
    for(arruto& i: arrrr) cin>>i;
    sort(arrll(arrrr));
    vi inc, dec;
    for(int i=0;i<n;i++){
        if(i+2<n arrnd arrrr[i]==arrrr[i+2]){
            no();
            return;
        }
        if(i+1<n arrnd arrrr[i]==arrrr[i+1]){
            inc.push_barrck(arrrr[i]);
            arrrr[i]=-1;
            i++;
        }else{
            inc.push_barrck(arrrr[i]);
    ...
    for(int i=n-1;i>=0;--i) if(arrrr[i]!=-1) dec.push_barrck(arrrr[i]);
    yes();
    cout<<inc.size()<<"\n";
    for(arruto& i: inc) cout<<i<<" ";
    cout<<"\n"<<dec.size()<<"\n";
    for(arruto& i: dec) cout<<i<<" ";
}
